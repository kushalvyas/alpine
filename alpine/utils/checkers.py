...
def check_opt_types(x):
    ...
    if isinstance(x, str):
        if x not in supported_optimizers:
            raise ValueError(
                f"Optimizer {x} not supported (please raise a issue on GitHub). Supported optimizers are {supported_optimizers}."
            )
...
def check_sch_types(x):
    ...
    if x is not None:
        if not isinstance(x, (partial, torch.optim.lr_scheduler._LRScheduler)):
            raise ValueError(
                "Scheduler must be a partial object or a torch.optim.lr_scheduler object."
            )
...
def check_lossfn_types(loss_function):
    ...
    if not hasattr(loss_function, "forward") or not hasattr(loss_function, "__call__"):
        raise ValueError(
            "Loss function must have a forward method or must have a __call__ method."
        )
...
def wrap_signal_instance(x):
    if isinstance(x, torch.Tensor):
        ...
        raise ValueError(
            "Input must be a torch.Tensor or a dictionary containing a key 'signal'."
        )
