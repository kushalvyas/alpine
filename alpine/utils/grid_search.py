...
class GridSearch:
    ...
    def run(self, coords, signal, verbose=True):
        self.results = []
        ...
        print(f"Running grid search with {num_iterations} total runs.")
        ...
        for model_params in model_param_combos:
            for compile_params in compile_param_combos:
                for n_iters_param in self.n_iters:
                    ...
                    print(f"Training with model params: {model_params}, compile params: {compile_params}, n_iters param: {n_iters_param}")
