...
class GridSearch:
    ...
    def run(self, coords, signal, metric_trackers=None, verbose=True):
        ...
        if verbose:
            print(f"Starting {total_runs} runs on {self.device}. Saving to {self.save_path}")
            ...
            for c_params in compile_combos:
                for n_iters in self.n_iters:
                    try:
                        ...
                        if verbose:
                            config_str = f"Arch:{m_params} | Opt:{c_params} | Iters:{n_iters}"
                            outer_pbar.write(f"Running: {config_str}")
                            
                        # Configure Inner Bar
                        kwargs = {}
                        if verbose:
                            kwargs = {'tqdm_kwargs': {'position': 1, 'leave': False, 'desc': 'Training'}}
                        ...
                        if final_loss < self.best_loss:
                            ...
                            self.best_config = {**m_params, **c_params, 'n_iters': n_iters}
                            # Deepcopy to CPU so it persists after 'model' is deleted
                            self.best_state_dict = {k: v.cpu().clone() for k, v in model.state_dict().items()}

                    except Exception as e:
                        if verbose: outer_pbar.write(f"--> FAILED: {e}")
                    ...
                    # Visual Update
                    if verbose:
                        outer_pbar.set_postfix({'Best': f"{self.best_loss:.6f}", 'Curr': f"{final_loss:.6f}"})
                        outer_pbar.update(1)
