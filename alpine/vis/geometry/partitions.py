...
def plot_partitions(
    partition_images_set, normalize_each=False, title="", figsize=(10, 15), dpi=200
):
    """_summary_
    ...
    def get_partitions_from_inr(
        ...
        target_ids = np.asarray(
            [
                ...
                if (type(each) == torch.nn.modules.Linear)
            ]
        )
        ...
        layer_names = np.sort(
            np.asarray(names)[target_ids]
        )  # TODO: replace with a non-sort based method but a better traversal of model tree
        ...
        for idx, coord_batch in enumerate(coords_batched):
            hull = get_ortho_hull_around_samples(
                torch.Tensor(coord_batch[..., None, None]),
                r=partitions.r,
                n=partitions.n,
            ).squeeze()
            hulls.append(hull)
        ...
        intersections_reshaped = [
            intersections[..., i].reshape(sampled_points, sampled_points)
