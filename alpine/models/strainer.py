...
def get_linear_layer(
    in_features, out_features, omega=30.0, bias=True, is_first=False, is_last=False
):
...
class Strainer(AlpineBaseModule):
    def __init__(
        ...
        super().__init__()
        assert (
            hidden_layers > num_shared_layers
        ), "Number of shared layers should be less than total number of layers"
        ...
        for i in range(self.num_decoders):
            ...
            for j in range(num_shared_layers, hidden_layers):
                _decoder.append(
                    get_linear_layer(
                        hidden_features,
                        hidden_features if j != hidden_layers - 1 else out_features,
                        is_first=False,
                        omega=self.omegas[j],
                        bias=bias,
                        is_last=(j == hidden_layers - 1),
                    )
                )
