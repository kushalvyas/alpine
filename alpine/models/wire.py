...
def get_linear_layer(
    in_features, out_features, bias=True, is_first=False, is_last=False
):
    if is_first:
        dtype = torch.float
    else:
...
class Wire(AlpineBaseModule):
    def __init__(
        ...
        self.sigmas = (
            sigmas if len(sigmas) == hidden_layers else [sigmas[0]] * (hidden_layers)
        )

        self.model.append(
            get_linear_layer(in_features, hidden_features, is_first=True, bias=bias)
        )
        ...
        for i in range(hidden_layers - 2):
            self.model.append(
                get_linear_layer(
                    hidden_features, hidden_features, is_first=False, bias=bias
                )
            )
            self.model.append(
                Wavelet(omega=self.omegas[i + 1], sigma=self.sigmas[i + 1])
            )
