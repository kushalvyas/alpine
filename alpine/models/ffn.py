...
class PosEncoding(nn.Module):
    def __init__(self, in_features, mapping_size, scale):
        ...
        self.B = nn.Parameter(
            torch.randn(self.mapping_size, self.in_features) * scale,
            requires_grad=False,
        )
...
class FFN(AlpineBaseModule):
    def __init__(
        ...
        positional_encoding_kwargs={"mapping_size": 256, "scale": 10.0},
    ):
        ...
        if self.positional_encoding is not None:
            self.pos_encode = PosEncoding(
                in_features=in_features, **positional_encoding_kwargs
            )
