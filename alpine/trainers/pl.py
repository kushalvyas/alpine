...
class LightningTrainer(pl.LightningModule):
    ...
    def train_dataloader(self):
        ...
        ), "Dataloader is not set. Please set the dataloader using the set_dataloader method."
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.closure is None:
            ...
            output_packet = self.closure(
                self.model, input, signal, return_features=self.return_features
            )
    ...
    def on_test_end(self):
        print(
            f"[on_test_end] Local rank: {self.local_rank}, Global rank: {self.global_rank}"
        )
        if self.trainer.is_global_zero:
