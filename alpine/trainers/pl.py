...
class LightningTrainer(pl.LightningModule):
    def __init__(self, model, closure = None, return_features = False, logger = False):
        """
        ...
        self.closure = self.model.forward if closure is None else closure # set closure to model forward
    ...
    def training_step(self, batch, batch_idx):
        ...
        # Forward pass
        output_packet = self.closure(self.model, input,return_features=self.return_features)
        
        # Loss calculation
        loss = self.loss_function(output_packet, signal)
        self.log("train_loss", loss, prog_bar=True, on_epoch=True, on_step=True, logger=self.logger)
        return loss
