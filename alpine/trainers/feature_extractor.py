...
class FeatureExtractor:
    ...
    def _register_hooks_recursively(self, module: nn.Module):
        ...
        for name, child in module.named_children():
            ...
            if child_feature_dict or is_pre_layer or is_post_layer:
                ...
                if is_pre_layer:
                    self._attach_hook(
                        child_feature_dict, "pre_activation", child, "pre"
                    )
                if is_post_layer:
                    self._attach_hook(
                        child_feature_dict, "post_activation", child, "post"
                    )
    ...
    def _attach_hook(
        self, feature_dict: OrderedDict, name: str, child: nn.Module, hook_type: str
    ):
        handle = child.register_forward_hook(
            self._make_hook(feature_dict, name, hook_type)
        )
