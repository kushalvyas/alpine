...
class AlpineBaseModule(nn.Module):
    ...
    def fit_signal(
        ...
        *,
        ...
        if dataloader is not None and (input is not None or signal is not None):
            raise ValueError(
                "Either dataloader or input and signal pair must be provided, not all."
            )
        if dataloader is None:
            if input is None or signal is None:
                raise ValueError(
                    "Either dataloader or input and signal pair must be provided."
                )
    ...
    def _fit_signal_tensor(
        ...
        signal = wrap_signal_instance(
            signal
        )  # triggers if signal is a torch.Tensor. Alpine's workflow is with dict-based and not tensor based.
        loss_history = []

        iter_pbar = (
            range(n_iters)
            if not enable_tqdm
            else tqdm(range(n_iters), **kwargs.get("tqdm_kwargs", {}))
        )
        for iteration in iter_pbar:
            ...
            loss = self.loss_function(
                output_packet, signal
            )  # loss function takes in the output packet and the signal.
            ...
            if enable_tqdm:
                ...
                iter_pbar.set_description(
                    f"Iteration {iteration}/{n_iters}.  Loss: {loss.item():.6f}. {best_loss_str}"
                )
                iter_pbar.refresh()
    ...
    def _fit_signal_dataloader(
        ...
        for iteration in iter_pbar:
            ...
            if enable_tqdm:
                iter_pbar.set_description(
                    f"Iteration {iteration}/{n_iters}.  Loss (over dataloader): {loss_over_dl:.6f}"
                )
    ...
    def render(
        self, input, closure=None, return_features=False, use_best_weights=False
    ):
        ...
        with torch.no_grad():
            if closure is not None:
                output_quantities = closure(
                    self, input, return_features=return_features
                )
            else:
                if return_features:
