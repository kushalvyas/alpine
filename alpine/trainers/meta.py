...
class MAML(nn.Module):
    ...
    def setup_init_lr(self, init_lr):
        if self.lr_type == "static":
            ...
            self.lr = nn.ParameterList(
                [nn.Parameter(torch.Tensor([init_lr])) for _ in range(self.inner_steps)]
            )
            ...
            self.lr = nn.ParameterList(
                [nn.Parameter(torch.Tensor([init_lr])) for _ in self.mdoel.parameters()]
            )
        ...
        if batch_size > 1:
            ...
            def forward_single(params_dict, coords_single):
                return torch.func.functional_call(
                    self.model, params_dict, (coords_single,)
                )

            batched_out = torch.func.vmap(forward_single, in_dims=(0, 0))(
                params, coords
            )
    ...
    def generate_params(
        ...
        **kwargs,
        ...
        with torch.enable_grad():
            ...
            ):  # meta_named_parameters() #TODO: will replace alpine.base with two version, one for normal and one for meta.s
                adapted_parameters[name] = param[None, ...].repeat(
                    (meta_batch_size,) + (1,) * len(param.shape)
                )
            ...
            for j in range(num_meta_steps):
                ...
                for i, ((name, param), grad) in enumerate(
                    zip(adapted_parameters.items(), grads)
                ):
                    if self.lr_type in ["static", "global"]:
                        ...
                        lr = (
                            self.lr[i][j]
                            if num_meta_steps <= self.num_meta_steps
                            else 1e-2
                        )
    ...
    def fit_signal(
        ...
        *,
        ...
        """
        assert (
            input is not None and signal is not None
        ), "Input and signal must be provided"
    ...
    def _fit_signal_tensors(
        ...
        params = self.generate_params(input, signal, num_meta_steps=self.inner_steps)
